...
def create_ema_model(model):
    ...
    """
    ema_model = Network(config.num_classes,criterion=criterion, norm_layer=nn.BatchNorm2d)
...
with Engine(custom_parser=parser) as engine:
    ...
    feature_memory = FeatureMemory(num_samples = config.num_train_imgs, dataset='pascal_voc', memory_per_class = 256, feature_size=256, n_classes= num_classes)
    ...
    for epoch in range(engine.state.epoch, config.nepochs):
        ...
        # Save the model
        if (epoch > 15) and (epoch % config.snapshot_iter == 0) or (epoch == config.nepochs - 1):
