...
from furnace.utils.pyt_utils import parse_devices
...
class SegEvaluator(Evaluator):
    def func_per_iteration(self, data, device):
        ...
        if self.save_image:
            ...
            # 把真实标签255的也变成255 白色
            # pred[np.where(label == 255)] = 255
            comp_img = show_img(colors, config.background, image, clean, label, pred)
            # cv2.imwrite默认保存为BGR格式，因为OpenCV中的颜色通道顺序是BGR。
...
if __name__ == "__main__":
    ...
    parser.add_argument('--save_path', '-p', default=config.log_dir+'/pred_imgs_train/')
